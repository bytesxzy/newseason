...
(function (root) {
  ...
  /* A sentence may open with a year or a number ("2016 was designated..."),
     which the capital-letter boundary does not see; without this the rest of
     an article collapses into its first sentence. */
  function splitLeadingNumbers(list) {
    var out = [];
    for (var i = 0; i < list.length; i++) {
      var parts = String(list[i]).split(/(?<=[a-z)\]][.!?])\s+(?=\d{3,4}\b)/);
      for (var j = 0; j < parts.length; j++) if (parts[j].trim().length > 15) out.push(parts[j].trim());
    }
    return out;
  }
  function propositionsFrom(title, text, source, sourceKind) {
    var out = [], ss = splitLeadingNumbers(sentences(text)), m;
    for (var i = 0; i < ss.length && i < 14; i++) {
      ...
      if ((m = s.match(/^(?:The\s+|An?\s+)?(.{2,80}?)\s+(is|are|was|were)\s+(.{6,300}?)[.]/))) {
        out.push(new Proposition({
          ...
          /* the source's own tense: "2016 WAS a leap year" stays past */
          qualifiers: { copula: m[2].toLowerCase() },
