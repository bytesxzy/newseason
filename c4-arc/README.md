...
## Residual-driven refinement (modules 55-58, kernel in `../c4-reason-kernel.js`)

The manifest compiles `../../c4-reason-kernel.js` into the bundle, so the ARC
engine and the language stack share one kernel source.

- `55-residual.js`: structured residuals. Quick pass (cell errors, normalised
  error, bits, under/excess/wrong change relative to the input, invariant
  violations, a coarse signature for clustering) and full diagnosis (dims
  relation: orientation/scale/tile/crop/extend; consistent colour
  substitution; whole-grid orientation/translation; error regions; object
  correspondence: moved/recoloured/reoriented/missing/extra, count, topology,
  relation flips; the input-object class that separates mishandled from
  handled objects in every pair; the change-set class that separates
  correct from excessive change components). Diagnoses are `strong` only
  when consistent across demonstrations.
- `56-repair.js`: typed program repair. Near-misses are `base >> tree`
  (an existing hypothesis followed by a PROG tree; typed near-misses have no
  base, so every node is editable). Mutations: parameter substitution,
  operator substitution, predicate inversion, composition insertion/
  deletion/reorder, literal colour -> semantic role, selected <-> forall
  objects, object-class restriction, change masks, change-set filtering,
  fitted closers (colour tables, object tables, context tables keyed by
  input colour or neighbour count). Repair-only operators live in an
  auxiliary PROG alphabet behind a one-bit escape, so existing code lengths
  and rankings are unchanged.
- `57-refinement.js`: the portfolio stage. Failing candidates leave a bounded
  O(area) record per module (`_candidates` -> `noteNear`); typed synthesis
  hands over its closest non-exact beam states. The kernel runs on time the
  schedule left unused (plus the evaluation reserve when nothing executable
  exists). Only programs that reproduce every demonstration and execute on
  every test input are emitted, as family `repair`; repairs that duplicate
  an existing exact prediction are dropped as redundant evidence. Repaired
  parameters get leave-one-out refits.
- `57a-repair-policy.js`: generated by `tools/arc-curriculum.js` from a
  self-generated curriculum (no ARC data): repair ordering by diagnosis and
  meta-controller weights by state feature.
- `58-counterfactual.js`: when exact explanations disagree about the test,
  label-free probes (colour swap, object removal, nudge, swap) measure
  fragility and divergence; a bounded fragility cost (4 bits) is the only
  effect on ranking.

Tests: `node c4-arc/kernel-test.js`.
